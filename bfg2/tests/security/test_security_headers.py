...
def _directives(csp):
    """Split a CSP header value into a set of whole-directive strings.

    Whole-directive comparison dodges CodeQL's ``py/incomplete-url-
    substring-sanitization`` warning and is strictly stronger than a
    bare substring check: it fails if an allow-listed origin migrates
    into the wrong directive.
    """
...
class TestBuildCsp:
    def test_default_api_origins_collapses_to_self(self):
        ...
        assert (
            "connect-src 'self' 'self' https://api.stripe.com https://*.sentry.io"
            in _directives(build_csp(None))
        )

    def test_extra_api_origins_are_joined(self):
        directives = _directives(
            build_csp(["https://api.example.com", "https://api-b.example.com"])
        )
        assert (
            "connect-src 'self' https://api.example.com https://api-b.example.com "
            "https://api.stripe.com https://*.sentry.io"
            in directives
        )

    def test_all_required_directives_present(self):
        directives = _directives(build_csp(None))
        required = {
            "default-src 'self'",
    ...
    def test_frame_ancestors_none_blocks_iframe_embedding(self):
        # Defence against clickjacking of the backend (e.g. admin pages)
        # from third-party frames.
        assert "frame-ancestors 'none'" in _directives(build_csp(None))
...
class TestHeaderOutput:
    ...
    def test_csp_value_includes_stripe(self, rf):
        mw = _make_middleware(env="prod")
        response = mw(rf.get("/"))
        directives = _directives(response["Content-Security-Policy"])
        # Exact-directive matching — see _directives() docstring for why
        # we can't just substring-check ``"https://js.stripe.com" in ...``.
        assert "script-src 'self' 'unsafe-inline' https://js.stripe.com" in directives
        assert "frame-src https://js.stripe.com https://hooks.stripe.com" in directives
...
class TestIntegration:
    ...
    # A PUBLIC_PATH prefix so WorkspaceMiddleware (strict since PR-04)
    # lets the request through. Whether the underlying view exists is
    # irrelevant — SecurityHeadersMiddleware runs on every response,
    # including 404s.
    PUBLIC_URL = "/api/v1/health/__probe__/"

    def test_headers_attached_to_response(self):
