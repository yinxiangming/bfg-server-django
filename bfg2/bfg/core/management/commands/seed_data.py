...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--copy-images',
            action='store_true',
            help='Copy seed images from bfg seed_media to media/seed_images',
        )
        parser.add_argument(
            '--module',
            type=str,
    ...
    def handle(self, *args, **options):
        ...
        elif not self._seed_images_ready():
    ...
    def _seed_images_ready(self):
        """Return True if seed_images/store exists and contains at least one file."""
        store_dir = os.path.join(settings.MEDIA_ROOT, 'seed_images', 'store')
    ...
    def copy_sample_images(self):
        """Copy seed images from bfg seed_media (or BFG_SEED_MEDIA_ROOT) to media/seed_images.
        Ensures shop seed_data finds product/category images at seed_images/store/.
        """
        import bfg
        # Prefer explicit setting so it works when bfg is installed outside the repo
        seed_media_root = getattr(
            settings,
            'BFG_SEED_MEDIA_ROOT',
            os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(bfg.__file__))), 'seed_media'),
        )
        target_dir = os.path.join(settings.MEDIA_ROOT, 'seed_images')

        if not os.path.isdir(seed_media_root):
            self.stdout.write(self.style.WARNING(
                f'Seed media source not found: {seed_media_root}. '
                'Set BFG_SEED_MEDIA_ROOT in settings to point at bfg2/seed_media.'
            ))
            return

        os.makedirs(target_dir, exist_ok=True)
        for name in os.listdir(seed_media_root):
            src = os.path.join(seed_media_root, name)
