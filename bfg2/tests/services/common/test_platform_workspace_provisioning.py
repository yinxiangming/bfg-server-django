"""
Workspaces created through the platform API, ``POST /api/v1/platform/workspaces/``.

The view creates the workspace and queues the ``provision_workspace`` task for the
rest. The test settings would run that task eagerly, inside the request, but a
worker runs it later with no workspace bound to the thread. So ``.delay`` is
captured here and the task is called directly once the thread is clear.
"""
...
def queued(monkeypatch):
    """The view's ``provision_workspace.delay`` calls, captured instead of sent.

    Patched where the view looks the task up. The task is a proxy to the current
    Celery app's copy of it, and the app a request runs under need not be the
    one current when this fixture runs.
    """
    from bfg.platform.views import workspace_views

    calls = []
    monkeypatch.setattr(
        workspace_views, 'provision_workspace', SimpleNamespace(delay=lambda **kwargs: calls.append(kwargs)),
    )
    return calls
...
def test_provisioning_gives_the_workspace_its_notification_templates(queued):
    ...
    # The platform API sets no locale, so they are written in the settings defaults.
