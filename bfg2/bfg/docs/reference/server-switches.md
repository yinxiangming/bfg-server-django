...
- **Two priced add-ons are switched on before any bill**, and the answer says which: `trial_days` when the plan carries `trial_period_days` and this workspace has never held the add-on, and `billed_later` when no exchange rate had been stored to write today's bill at. Both write a bought period with the plan on it, so `issue_monthly_bills` charges for it in the month it ends and paying that renews it — what moves is when the first bill is written, not whether one is. A trial is had once: any entitlement row for the key, whatever became of it, means the workspace has had the add-on before.
- Refused with 400 and a `code`: `already_entitled`, `not_an_addon` (it is part of the base plan), `no_plan`, and — for a bill that cannot be written — `no_owner`, `unknown_currency`, `key_too_long`. 404 `unknown_extension` for a key no app declares.
...
- **What a console shows an add-on costs** is the `price` on each extension in the console payload: `plan` and `amount`/`currency` from the plan itself, `workspace_amount`/`workspace_currency` for the same month in the workspace's own currency (null when no rate has been stored, so a price is still shown), `interval`/`interval_count`, and `trial_days` — what *this* workspace would get, which is zero once it has held the add-on. Null for an extension that comes with the base plan and for one nothing prices.
...
There is no scheduler in this library. The steps below are management commands, to be run from cron, by hand, or by a Celery beat the deployment runs itself — `bfg.platform.tasks` wraps the first three and `bfg.common.tasks.archive_unused_extensions` the fourth, each a thin call to the same service the command calls, so a deployment with a scheduler and one without do the same thing. All of them are safe to run twice.

Order matters once a month: refresh the rates **before** issuing bills. A bill converts at the rate on file, and a refresh that failed leaves yesterday's.
...
A platform bill is an ordinary `finance.Invoice` belonging to the platform workspace, so it is settled the way every other invoice on the deployment is — there is no separate collection path to keep in step.

- **The console starts it.** `POST /api/v1/platform/console/workspaces/{id}/invoices/{number}/pay/`, with an optional body `{"gateway": <id>}`, raises a `finance.Payment` against the bill and answers with the gateway's own payload and whatever that gateway publishes for a payer to act on. Reached by the same people the rest of the console is — the workspace's owner, and platform administrators — and by them alone: the bill's number must begin with that workspace's own prefix, so another tenant's bill is answered `404 invoice_not_found` exactly as a number nobody holds is.
- **The gateway that takes it is the platform workspace's**, not the billed workspace's: the deployment is what is being paid. With no `gateway` named, the lowest-numbered active one is used, which is the only one on a deployment configured with a single way to be paid.
- **Starting a payment settles nothing.** The payment comes back `pending`. A card is confirmed by the gateway's callback (`POST /api/v1/store/payments/callback/{gateway}/`, signature and all) and an offline payment — a bank transfer, paying at the counter — by whoever reconciles it, through `POST /api/v1/finance/payments/{id}/process/` or by marking the invoice paid. Money is never taken on the payer's word.
- **Settling it is what renews.** However the money arrives, `bfg.platform.handlers` hears the invoice reach `paid` and `services.renewals` writes the next period of everything that bill charged a renewal for. It also drops both of the cached answers that stop a workspace — whether it owes anything, and whether it may be written to at all — so a workspace is trading again on its next request rather than a minute later.
- **Refusals all carry a `code`** and none of them is a server error: `invoice_not_found`, `invoice_already_paid`, `invoice_cancelled`, `invoice_nothing_to_pay` (a month covered entirely by the trial credit is a record, not a debt), `no_payment_gateway`, `payment_gateway_not_found`, `payment_gateway_unavailable`, `payment_in_progress` (an attempt is already under way that this request does not match — finish or reconcile that one first), and `platform_workspace_missing` as a 503. Asking twice for the same gateway hands back the attempt already started rather than raising a second one.
...
- Behavior:
  ...
  - The answer is cached for a minute (`bfg.platform.services.read_only.CACHE_SECONDS`), like the overdue-invoice answer. Whatever settles a payment should call `read_only.forget(workspace)` so the shop is trading on the next request rather than in a minute's time; `entitlements.grant` and `services.renewals` both already do.
